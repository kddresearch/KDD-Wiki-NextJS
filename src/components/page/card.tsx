...
function Card({ title = undefined, actions = undefined, isFlex = true, ...props }: { title?: string } & {actions?: ReactNode} & {isFlex?: boolean} & React.HTMLProps<HTMLDivElement>) {
  
  let className;

  if (isFlex) {
    className = 'flex flex-col container p-5 bg-white text-lg my-8 ' + props.className;
  } else {
